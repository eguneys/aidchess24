...
export class Treelala {
  ...
  try_next_uci_fail = (uci: string) => {
    ...
    const cc = a0.children

    if (cc[0]?.data.uci === uci) {
      let hh = this.hidden_paths
      hh = hh.filter(_ => _.join('') !== cc[0].data.path.join(''))
      let cc0cc = cc[0].children.map(_ => _.data.path)
      hh.push(...cc0cc)
      this.hidden_paths = hh
      let rr = this.solved_paths
      rr.push(cc[0].data.path)
      this.solved_paths = rr
      this.cursor_path = cc[0].data.path
    } else {
...
const RenderData = (props: { on_set_path: (_: string[]) => void, 
  ...
  failed_paths: string[][], 
  hidden_paths: string[][], 
  cursor_path: string[], data: MoveData, show_index?: true }) => {
