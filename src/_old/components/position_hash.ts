import { COLORS, Position, ROLES, squareFile } from "chessops"

export type Hash = number
export type PositionHash = Hash[]


export function combine_hash(a: PositionHash, b: PositionHash) {
    return [...a, ...b]
}
...
export function hash(p: Position): Hash {
    return hPieces(p) ^ hTurn(p) ^ hCastling(p) ^ hEp(p)
}


function hPieces(p: Position): Hash {

    let h = 0
    COLORS.forEach(color => {
        let c = p.board[color]
        let colorSub = actorMasks[color]
        ROLES.forEach(role => {
            let r = p.board[role]
            let sub = colorSub[role]

            for (let s of c.intersect(r)) {
                h ^= sub[s]
            }
        })
    })
...
function hTurn(p: Position): Hash {
    return p.turn === 'white' ? whiteTurnMask : 0
}
function hCastling(p: Position): Hash {
    let res = 0
    if (p.castles.rook['white']['a'] !== undefined) {
        res ^= castlingMasks.white[0]
    }
    if (p.castles.rook['white']['h'] !== undefined) {
        res ^= castlingMasks.white[1]
    }
    if (p.castles.rook['black']['a'] !== undefined) {
        res ^= castlingMasks.black[0]
    }
    if (p.castles.rook['black']['h'] !== undefined) {
        res ^= castlingMasks.black[1]
    }
    return res
}
function hEp(p: Position): Hash {

    if (p.epSquare !== undefined) {
        return enPassantMasks[squareFile(p.epSquare)]
    }
    return 0
}
